...
export default function MobileBarPage() {
  const [heroData, setHeroData] = useState({
    ...
    description: 'Bringing the party to you with professional bartending and premium drinks',
    backgroundImage: '',
  ...
  // Load hero settings and service data from API
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.success && result.data) {
            console.log('[MOBILE-BAR] Loaded service settings from API:', result.data);
            setServiceData({
              packages: result.data.packages || [],
              features: result.data.features || [],
              title: result.data.title || 'Mobile Bar Service',
              subtitle: result.data.subtitle || 'We Come to You',
              description: result.data.description || 'Bringing the party to you with professional bartending and premium drinks',
            });
          }
        }
      } catch (error) {
  ...
  // Use dynamic data from API, fallback to hardcoded if not available
  const features = serviceData.features.length > 0 ? serviceData.features.map((feature: any) => ({
  ...
  const packages = serviceData.packages.length > 0 ? serviceData.packages : [
    {
      id: 'basic',
      name: 'Essential Bar',
