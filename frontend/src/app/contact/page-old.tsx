...
export default function ContactPage({ settings: serverSettings }: ContactPageProps) {
  ...
  // Fetch settings on client side if not provided by server
  useEffect(() => {
    if (!serverSettings) {
      const fetchSettings = async () => {
        try {
          ...
          if (response.ok) {
            ...
            setSettings({
              restaurantName: data.siteName || "Kocky's Bar & Grill",
              tagline: data.siteDescription || "Where Great Food Meets Unforgettable Moments",
              address: data.address || "123 Main Street, City, State 12345",
              phone: data.contactPhone || "(555) 123-4567",
              email: data.contactEmail || "info@kockys.com",
              website: data.onlineOrderingUrl || "https://kockys.com",
              socialMedia: {
                facebook: "https://facebook.com/kockys",
                instagram: "https://instagram.com/kockys",
                twitter: "https://twitter.com/kockys",
  ...
  const contactInfo = [
    ...
    },
    {
      icon: Phone,
      title: 'Call Us',
      lines: [settings.phone, 'Mon-Sun: 10AM-10PM'],
    },
    {
      icon: Mail,
      title: 'Email Us',
