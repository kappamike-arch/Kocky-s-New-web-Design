...
export default function HomePageClient({ settings, heroSettings: initialHeroSettings }: HomePageClientProps) {
  console.log('HomePageClient: Received settings:', settings?.contactPhone, settings?.address);
  ...
  const address = settings?.address || "123 Main Street, Downtown";
  
  console.log('HomePageClient: Using phone:', phone, 'address:', address);
  ...
  // Hero settings state - use server data initially
  const [heroData, setHeroData] = useState({
    title: initialHeroSettings?.title || "Welcome to Kockys Bar & Grill",
    subtitle: initialHeroSettings?.subtitle || 'Amazing Food & Drinks',
    description: initialHeroSettings?.description || 'Come experience the best dining in town',
    useLogo: initialHeroSettings?.useLogo !== false,
    logoUrl: initialHeroSettings?.logoUrl || '/kockys-logo.png'
  });
  const [heroLoaded, setHeroLoaded] = useState(false);
  const [heroSettings, setHeroSettings] = useState<any>(initialHeroSettings);
