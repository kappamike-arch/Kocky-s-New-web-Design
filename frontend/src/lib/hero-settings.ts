...
// Cache duration in milliseconds (5 minutes for better performance)
...
async function fetchHeroSettings(pageId: string): Promise<HeroSettings | null> {
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/hero-settings/${pageId}`, {
      ...
      params: {
        timestamp: Date.now() // Force fresh fetch
      }
    });
    if (response.data && response.data.id) {
      if (process.env.NODE_ENV === 'development') console.log(`Received settings for ${pageId}:`, response.data);
      // Also save to localStorage for persistence
      if (typeof window !== 'undefined') {
        localStorage.setItem(`heroSettings_${pageId}`, JSON.stringify(response.data));
      }
...
export async function getHeroSettingsAsync(pageId: string): Promise<HeroSettings | null> {
  ...
  try {
    ...
    const apiUrl = addCacheBuster(`https://staging.kockys.com/api/hero-settings/${pageId}`);
    ...
    if (result && result.id) {
      ...
      localStorage.setItem(`hero-settings-${pageId}`, JSON.stringify(settings));
      
      if (process.env.NODE_ENV === 'development') console.log(`[DEBUG] Fresh settings stored for ${pageId}:`, settings);
      return settings;
    }
