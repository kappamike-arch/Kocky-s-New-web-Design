...
export const heroSettingsAPI = {
  ...
  // Upload hero image for a specific page
  uploadImage: async (pageId: string, file: File): Promise<{ imageUrl: string }> => {
    try {
      const formData = new FormData();
      formData.append('image', file);
      const response = await api.post(`/hero-settings/${pageId}/upload-image`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      return response.data?.data;
    } catch (error) {
      console.error(`Failed to upload image for ${pageId}:`, error);
      throw error;
    }
  },
