...
export default function GalleryPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await fetch(`http://72.167.227.205:5001/api/gallery/bulk-delete`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${Cookies.get('auth-token')}`
  ...
  const handleDragEnd = async (event: any) => {
    ...
    if (active.id !== over.id) {
      ...
      try {
        ...
        const response = await fetch(`http://72.167.227.205:5001/api/gallery/items/reorder`, {
          method: 'PUT',
          headers: { 
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${Cookies.get('auth-token')}`
