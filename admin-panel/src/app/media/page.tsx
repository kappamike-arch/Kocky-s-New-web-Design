...
export default function MediaManagementPage() {
  ...
  const saveMediaSelection = async (sectionId: string, selection: 'image' | 'video' | 'auto') => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://72.167.227.205:5001/api'}/hero-settings/${sectionId}`, {
        method: 'PUT',
        headers: {
  ...
  useEffect(() => {
    const checkExistingFiles = async () => {
      for (const section of HERO_SECTIONS) {
        ...
        try {
          const imageResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || 'http://72.167.227.205:5001'}${section.imagePath}`, { method: 'HEAD' });
          if (imageResponse.ok) {
            setUploadedFiles(prev => ({
              ...prev,
              [`${section.id}-image`]: { 
                url: `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || 'http://72.167.227.205:5001'}${section.imagePath}`,
                timestamp: Date.now()
              }
            }));
          }
        ...
        try {
          const videoResponse = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://72.167.227.205:3003'}${section.videoPath}`, { method: 'HEAD' });
          if (videoResponse.ok) {
            setUploadedFiles(prev => ({
              ...prev,
              [`${section.id}-video`]: { 
                url: `${process.env.NEXT_PUBLIC_SITE_URL || 'http://72.167.227.205:3003'}${section.videoPath}`,
                timestamp: Date.now()
              }
            }));
          }
  ...
  const handleFileUpload = async (sectionId: string, file: File, type: 'image' | 'video') => {
    ...
    try {
      ...
      if (type === 'video') {
        ...
        fileUrl = `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || 'http://72.167.227.205:5001'}${result.videoUrl}`;
      } else {
        ...
        fileUrl = `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || 'http://72.167.227.205:5001'}${result.imageUrl}`;
      }
