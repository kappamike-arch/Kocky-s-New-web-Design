  ...
  const handleSaveItem = async () => {
    try {
      ...
      const cleanedFormData = {
        ...itemForm,
        happyHourPrice: itemForm.happyHourPrice && itemForm.happyHourPrice > 0 ? itemForm.happyHourPrice : null,
        servingSize: itemForm.servingSize || null,
        preparationTime: itemForm.preparationTime || null,
      };
      
      let savedItem;
      if (editingItem) {
        // For updates, only send the fields that can be updated
        const { id, createdAt, updatedAt, section, orderItems, ...updateData } = cleanedFormData;
        
        // Only include image field if it has actually changed (not empty/null)
        if (!updateData.image || updateData.image === '') {
          delete updateData.image;
        }
        
        const response = await api.put(`/enhanced-menu/items/${editingItem.id}`, updateData);
            ...
            <CardContent className="pt-6">
              <div className="flex justify-between items-center gap-4">
                <div className="flex gap-2 flex-1">
                  ...
                  <select 
                    className="flex h-10 w-[200px] rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-400"
