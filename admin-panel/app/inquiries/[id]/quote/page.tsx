...
- Travel fees may apply for distant locations`,
  ...
  const fetchInquiry = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/crm/inquiries/${inquiryId}`);
  ...
  const handleSubmit = async (sendNow = false) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/crm/inquiries/${inquiryId}/quotes`, {
        method: 'POST',
      ...
      if (sendNow && data.id) {
        ...
        const sendResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/quotes/${data.id}/send`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
