...
export default function EmailTemplatesPage() {
  ...
  const initializeDefaultTemplates = async () => {
    try {
      const response = await fetch('${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/email-templates/initialize', {
        method: 'POST',
      });
      if (response.ok) {
  ...
  const previewTemplate = async (template: EmailTemplate) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/email-templates/${template.id}/preview`, {
        method: 'POST',
  ...
  const saveTemplate = async (template: EmailTemplate) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/email-templates/${template.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteTemplate = async (id: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/email-templates/${id}`, {
        method: 'DELETE',
      });
