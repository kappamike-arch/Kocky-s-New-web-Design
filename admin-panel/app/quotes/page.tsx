...
export default function QuotesPage() {
  ...
  const fetchQuotes = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/quotes?${params}`);
      if (response.ok) {
  ...
  const updateQuoteStatus = async (quoteId: string, status: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/quotes/${quoteId}/status`, {
        method: 'PUT',
  ...
  const sendQuote = async (quoteId: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/quotes/${quoteId}/send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
  ...
  const downloadPDF = async (quoteId: string, quoteNumber: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/quotes/${quoteId}/pdf`);
  ...
  const emailQuotePDF = async (quoteId: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/quotes/${quoteId}/pdf/email`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: 'Thank you for your inquiry. Please find attached our quote for your event.',
  ...
  const deleteQuote = async (quoteId: string) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api/quotes/${quoteId}`, {
        method: 'DELETE',
      });
