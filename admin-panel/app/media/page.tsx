...
export default function MediaManagementPage() {
  ...
  const saveMediaSelection = async (sectionId: string, selection: 'image' | 'video' | 'auto') => {
    try {
      // Update the backend with the media preference using centralized API
      const { api } = await import('../../src/lib/api');
      const response = await api.put(`/hero-settings/${sectionId}`, {
      ...
      console.error('Error saving media preference:', error);
  ...
  useEffect(() => {
    ...
    const handleGlobalDrop = (e: DragEvent) => {
      e.preventDefault();
      e.stopPropagation();
      console.log('🚫 Prevented global drop/download');
    };

    // Add global event listeners to prevent file downloads
  ...
  useEffect(() => {
    const checkExistingFiles = async () => {
      console.log('🔍 Loading existing uploads from API...');
      
      // Check for existing uploads from the backend API
      
      for (const section of HERO_SECTIONS) {
        // Image paths will be loaded from the API data below

        // Check backend API for existing uploads
        try {
          console.log(`🔍 Checking ${section.id} for uploads...`);
          const { api } = await import('../../src/lib/api');
          const response = await api.get(`/hero-settings/${section.id}`);
          if (response.status === 200) {
            const data = response.data;
            console.log(`📊 ${section.id} API data:`, data);
            
            // If video exists, add it to uploaded files
            if (data.backgroundVideo) {
              const videoKey = `${section.id}-video`;
              setUploadedFiles(prev => ({
                ...prev,
                [videoKey]: { url: `http://72.167.227.205:5001${data.backgroundVideo}`, timestamp: Date.now() }
              }));
              setUploadStatus(prev => ({ ...prev, [videoKey]: 'success' }));
            }
            
            // If image exists, add it to uploaded files
            if (data.backgroundImage) {
              const imageKey = `${section.id}-image`;
              setUploadedFiles(prev => ({
                ...prev,
                [imageKey]: { url: `http://72.167.227.205:5001${data.backgroundImage}`, timestamp: Date.now() }
              }));
              setUploadStatus(prev => ({ ...prev, [imageKey]: 'success' }));
            }

            // Set media selection based on what's available
            if (data.backgroundVideo && data.backgroundImage) {
              setMediaSelection(prev => ({ ...prev, [section.id]: prev[section.id] || 'video' }));
            } else if (data.backgroundVideo) {
              setMediaSelection(prev => ({ ...prev, [section.id]: 'video' }));
            } else if (data.backgroundImage) {
              setMediaSelection(prev => ({ ...prev, [section.id]: 'image' }));
            } else {
              setMediaSelection(prev => ({ ...prev, [section.id]: 'auto' }));
            }
          }
        } catch (err) {
          console.error(`❌ Failed to check uploads for ${section.id}:`, err);
        }
      }
    };
  ...
  const handleDrop = async (e: React.DragEvent, sectionId: string, type: 'image' | 'video') => {
    e.preventDefault();
    e.stopPropagation();
    setDragActive(prev => ({ ...prev, [`${sectionId}-${type}`]: false }));
    ...
    if (files && files[0]) {
      console.log(`🎯 Dropping ${type} file for ${sectionId}:`, files[0].name);
      // Reset status to allow replacement
      const key = `${sectionId}-${type}`;
      if (uploadStatus[key] === 'success') {
        console.log(`🔄 Allowing replacement for existing ${type}`);
        setUploadStatus(prev => ({ ...prev, [key]: 'idle' }));
      }
      await handleFileUpload(sectionId, files[0], type);
    }
  };
  ...
  const UploadZone = ({ section, type }: { section: any; type: 'image' | 'video' }) => {
    ...
    const path = type === 'image' ? `/uploads/images/${section.id}-hero.jpg` : `/uploads/videos/${section.id}-hero.mp4`;
    ...
    return (
      <div className="space-y-2">
        <div
          ...
          e.preventDefault();
          e.stopPropagation();
          handleDragEnter(e, section.id, type);
        }}
        onDragLeave={(e) => {
          e.preventDefault();
          e.stopPropagation();
          handleDragLeave(e, section.id, type);
        }}
        onDragOver={(e) => {
          e.preventDefault();
          e.stopPropagation();
        ...
        onDrop={(e) => {
          e.preventDefault();
          e.stopPropagation();
          handleDrop(e, section.id, type);
        }}
          onClick={() => {
            // Allow click upload for any status (including replacement)
            if (status === 'success') {
        ...
        >
          ...
          <div className="flex flex-col items-center space-y-3">
            ...
            <div className="bg-gray-100 px-3 py-1 rounded-full">
              <code className="text-xs">{type === 'image' ? 'Image Upload' : 'Video Upload'}</code>
            </div>
