...
const API_URL = '${process.env.NEXT_PUBLIC_API_URL?.replace("/api", "")}/api';
...
export default function QuoteConfigPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/quote-config/${deleteModal.type}/${deleteModal.item.id}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      if (data.success) {
  ...
  const saveEditChanges = async (formData: any) => {
    ...
    try {
      ...
      const url = isNew
        ...
        : `${API_URL}/quote-config/${editModal.type}/${editModal.item.id}`;
...
function EditModal({ open, type, item, onClose, onSave }: any) {
  ...
  useEffect(() => {
    if (item) {
      console.log('EditModal: Setting formData from item:', item);
      setFormData(item);
    } else {
      // Set default values for new items
      const defaults = getDefaultFormData(type);
      console.log('EditModal: Setting default formData:', defaults);
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      console.log('Price type changed from:', formData.priceType, 'to:', value);
