...
export class PDFService {
  ...
  public async generateQuotePDF(quote: QuoteWithDetails): Promise<{ buffer: Buffer; filename: string }> {
    return new Promise((resolve, reject) => {
      try {
        ...
        const serviceDetails = quote.serviceDetails ? 
          (typeof quote.serviceDetails === 'string' ? 
            JSON.parse(quote.serviceDetails) : quote.serviceDetails) : null;
        
        if (serviceDetails) {
          doc.moveDown();
          this.addSectionTitle(doc, 'Service Details');
          
          doc.fontSize(11)
             .font('Helvetica');
          
          Object.entries(serviceDetails).forEach(([key, value]) => {
            if (value && key !== 'specialRequests') {
  ...
  private addHeader(doc: any) {
    ...
    const contactY = 50;
    doc.text('(559) 217-5719', 400, contactY, { width: 145, align: 'right' });
    doc.text('info@kockysbar.com', 400, contactY + 15, { width: 145, align: 'right' });
    doc.text('123 Main Street', 400, contactY + 30, { width: 145, align: 'right' });
    doc.text('City, State 12345', 400, contactY + 45, { width: 145, align: 'right' });
  }
